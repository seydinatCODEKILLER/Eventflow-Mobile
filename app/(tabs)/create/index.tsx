...
import { useCreateEvent } from "@/src/lib/hooks/use-events";
...
const stepTitles = [
  { title: "Présentez votre event", sub: "Donnez envie aux gens de venir" },
  { title: "Où et quand ?", sub: "Lieu et horaires de l'événement" },
    ...
    <View className="flex-row gap-3 mt-4">
      ...
      >
        {isPending ? (
          ...
          <>
            <Text className="text-white font-bold text-sm">{nextLabel}</Text>
            {isLast ? (
...
export default function CreateEventScreen() {
  ...
  const [collected, setCollected] = useState<Partial<CreateEventFormValues>>(
    {},
  );
  ...
  const onSubmit = form3.handleSubmit((v) => {
    const final = { ...collected, ...v } as CreateEventFormValues;
    createEvent({
      title: final.title,
    ...
    (mode: "start" | "end", currentValue: Date | undefined) => {
      if (Platform.OS === "android") {
        DateTimePickerAndroid.open({
          ...
          onChange: (_, date) => {
            if (!date) return;
            // Après la date, ouvrir le picker d'heure
            DateTimePickerAndroid.open({
              value: date,
              mode: "time",
              onChange: (_, time) => {
                ...
                if (mode === "start") {
                  form2.setValue("startDate", combined);
                } else {
                  form2.setValue("endDate", combined);
                }
              },
            });
          },
        ...
        <View className="px-4">
          ...
          {step === 1 && (
            <View>
              ...
              {/* Description — TextInput multiline (FormInput ne supporte pas multiline) */}
              <View className="mb-4">
                <Text className="text-foreground text-sm font-semibold mb-1.5">
                  Description
                </Text>
          ...
          {step === 2 && (
            <View>
              ...
              <View className="mb-4">
                ...
                <Controller
                  ...
                  render={({ field: { value } }) => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                      className={`flex-row items-center bg-card border rounded-2xl px-4 h-12 gap-3 ${
                        form2.formState.errors.endDate
                          ? "border-destructive"
                          : "border-border"
                      }`}
                    >
                ...
                {form2.formState.errors.endDate && (
                  <Text className="text-destructive text-xs mt-1">
                    {form2.formState.errors.endDate.message}
                  </Text>
                )}
              </View>

              {/* iOS uniquement — picker inline */}
              ...
              <View className="flex-row items-start gap-2 bg-primary/5 border border-primary/10 rounded-2xl p-3.5 mb-4">
                ...
                <Text className="text-primary text-xs flex-1 leading-5">
                  La date de fin est optionnelle. Si non renseignée, event sera
                  marqué comme terminé manuellement.
                </Text>
              </View>
